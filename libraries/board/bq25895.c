...
status_t Init_BQ25895(void)
{
    ...
    res |= bq2589x_reset_chip();                // 0x14 复位芯片
    res |= bq2589x_disable_watchdog_timer();    // 0x07 禁用看门狗
    res |= bq2589x_init_device();               // 初始化设备

    // res |= bq2589x_disable_otg();
    res |= bq2589x_enable_otg();     //使能OTG模式
    res |= bq2589x_set_otg_volt(4900);          //设置USB OTG电压  5.1V
    res |= bq2589x_set_otg_current(2100);       //设置USB OTG电流  1.8A

    // res |= bq2589x_disable_ILIM_pin();
    res |= bq2589x_enable_max_charge(false);     //MaxCharge 握手使能
    // res |= bq2589x_set_input_volt_limit(4600);  //5.6v
    // res |= bq2589x_set_input_current_limit(3000);

    res |= bq2589x_adc_start(false);            //one short mode?
    res |= bq2589x_enable_charger();            //使能充电
    res |= bq2589x_set_chargevoltage(4208);     //设置充电电压 4.208V
    // res |= bq2589x_set_charge_current(2048);    //设置充电电流 5.056A 最大

    // res |= bq2589x_set_prechg_current(1024);
    // res |= bq2589x_set_term_current(1024);

    res |= bq2589x_set_watchdog_timer(160);
    // res |= bq2589x_pumpx_enable(0);
    // res |= bq2589x_force_ico();

    PRINTF("\r\n dump bq25895 reg:  \r\n");
    for(int i = 0; i < 0x14; i++) {

      status = bq2589x_read_byte(&val, i);
      PRINTF("sta:%d bq25895 reg:0x%02x  val[0x%x]  \r\n", status, i, val);
    }
