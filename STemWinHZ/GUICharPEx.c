...
static void GUI_GetDataFromMemory(const GUI_FONT_PROP GUI_UNI_PTR *pProp, U16P c)
{
  ...
  unsigned long foffset; 
	unsigned char t;
	unsigned char *mat;
  unsigned char size,csize;//字体大小
  unsigned short BytesPerFont;
	GUI_FONT EMWINFONT;
  ...
  else                                                                          
  {
		...
		{
			case 16:
				memcpy(GUI_FontDataBuf,(unsigned char*)(foffset+ftinfo.f16addr), csize);
				break;
			
			case 24:
				memcpy(GUI_FontDataBuf,(unsigned char*)(foffset+ftinfo.f24addr), csize);
				break;	
			
      case 32:
				memcpy(GUI_FontDataBuf,(unsigned char*)(foffset+ftinfo.f32addr), csize);
				break;
		} 
	}   	
}
