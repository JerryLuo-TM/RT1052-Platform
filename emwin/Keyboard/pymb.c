...
/*"Æ´ÒôÊäÈë·¨²éÑ¯Âë±í, Æ´ÒôË÷Òı±í(index)"*/ //Ë÷Òı±í(Æ´ÒôÖ¸Õë, Âë±íÖ¸Õë)
...
const char *PYSearch(char *strInput_py_str)
{
    ...
    /*** ×ª³ÉË÷Òı±íĞòºÅ'a' = 61; (a-z)-'a' = ×ÖÄ¸Ë³Ğò, 'a'-'a'=0; 'z'-'a'=25;***/
    cpHZ  = PY_index_headletter[strInput_py_str[0] - 'a'];  //²éË÷ÒıÊ××ÖÄ¸;
    count = index_number[strInput_py_str[0] - 'a'];         //²éË÷ÒıÏî³¤¶È;

//    cpHZedge = PY_index_headletter[strInput_py_str[0] - 'a' + 1];     /*ÉèÖÃÖ¸Õë½çÏŞ*/
//    printf("%x, %x\n", cpHZ, cpHZedge);

    strInput_py_str++;                              //ÒÑ¿É¶¨Î»Ë÷Òı±í; Ö¸ÏòÆ´Òô´®µÚ¶ş¸ö×ÖÄ¸*/

    while(count--)                                  /*Ë÷Òı±í²»³¬½ç*/
    {
        for(i = 0; i < cInputStrLength; i++)        //ÊäÈë´®ÓëË÷Òı±íÆ´ÒôÏî±È¶ÔËã·¨
        {
        ...
        {
            return (cpHZ->PY_mb);                   //·µ»Øºº×ÖÂë±íÊ×Ö¸Õë (´ËÊäÈë·¨¿ÉÊ¹ÓÃÂë±íÊ×µØÖ·+Âë±í½áÊø·û'\0',Ö±½Ó¶¨Î»Êä³ö·¶Î§,¸ü¼òµ¥Ò×ÓÃ)
        }
        cpHZ++;                                     //±È¶ÔË÷Òı±íÏÂÒ»Æ´ÒôÏî
    }
    return NULL;                                    //Õû¸öË÷Òı±íÏîÈ«²éÍê(²»°üÀ¨Ë÷Òı×Ü¼¯)/*ÎŞ¹û¶øÖÕ*/
}
